...
const DEFAULT_RETRIES = 2; // Reduced from 3 to prevent overwhelming
const RETRYABLE_STATUS_CODES = [429, 502, 503, 504];
const NON_RETRYABLE_CODES = [400, 401, 403, 404]; // Don't retry client errors
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      if (
        ...
        error.response &&
        NON_RETRYABLE_CODES.includes(error.response.status)
      ) {
        throw error;
      }
